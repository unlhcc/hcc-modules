help (
[[
This module makes available RGI 5.1.0 (Resistance Gene Identifier).
...
pushenv("CONDA_DEFAULT_ENV","rgi-5.1.0")
prepend_path{"PATH","/util/opt/anaconda/deployed-conda-envs/packages/rgi/envs/rgi-5.1.0/bin",priority=100}

local data = "/work/HCC/BCRF/app_specific/rgi/5.1"
local wild = pathJoin(data, "wildcard")

pushenv("CARD", data)
pushenv("CARD_JSON", pathJoin(data, "card.json"))
pushenv("CARD_ANNOT", pathJoin(data, "card_database_v3.0.5.fasta"))
...
-- Fix the libopenssl library load in the bioconda rgi=5.1.0 setup --
