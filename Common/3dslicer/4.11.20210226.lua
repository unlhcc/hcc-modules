...
if mode() == "load" then
  ...
  LmodMessage("3D Slicer does have some modules that can be run as CLI batch jobs. To list those, run 'ls $SLICER_CLI_MODULES'.")
  LmodMessage("For example, to use the BRAINSFit module, please run:")
  LmodMessage("Slicer --launch ${SLICER_CLI_MODULES}/BRAINSFit <options>")
  LmodMessage("---------------------------------------------------------------------------------------------")
end
...
setenv("SLICER_CLI_MODULES", "/util/opt/3dslicer/4.11.20210226/lib/Slicer-4.11/cli-modules/")
